...
class ThemeManager:
    ...
    def get_color(self, semantic_name: str) -> str:
        ...
        if (current_theme and 
            current_theme.name != "cyro" and 
            hasattr(current_theme.colors, semantic_name)):
            ...
            for theme_file in themes_path.glob("*.toml"):
                try:
                    ...
                    theme = Theme(
                        ...
                        description=theme_data.get("description", f"Custom theme: {theme_data['name']}"),
                        colors=colors
                    )
...
# Global theme manager instance for application-wide theming
_theme_manager = ThemeManager()


def get_theme_color(semantic_name: str) -> str:
    """
    Get a theme color by semantic name.
    
    This is the primary interface for accessing theme colors throughout the
    application. It provides a drop-in replacement for hardcoded colors and
    always returns a valid Rich color string.
    ...
    Examples:
        # Replace hardcoded colors with semantic names
        panel = Panel(title=f"[bold {get_theme_color('primary')}]Title[/]")
        console.print(f"[{get_theme_color('success')}]Success![/]")
    """
    return _theme_manager.get_color(semantic_name)


def set_theme(theme_name: str) -> bool:
    """
    Switch to a different theme globally.
...
def get_current_theme_name() -> str:
    ...
    """
    return _theme_manager.get_current_theme_name()
...
def get_theme_info(theme_name: str) -> Optional[Dict[str, str]]:
    """
    Get information about a specific theme.
    ...
    Example:
        info = get_theme_info("cyro")
        if info:
            print(f"{info['name']}: {info['description']}")
    """
    return _theme_manager.get_theme_info(theme_name)


def load_custom_themes(themes_dir: str) -> int:
    """
    Load custom themes from TOML files in a directory.
    
    This is a fail-safe operation that skips invalid themes and never
    affects the built-in themes or application stability.
    ...
    Example:
        count = load_custom_themes("~/.config/cyro/themes/")
        if count > 0:
            print(f"Loaded {count} custom themes")
    """
    return _theme_manager.load_custom_themes(themes_dir)


def reset_themes() -> None:
    """
    Reset to only built-in default themes.
    
    This removes all custom themes and resets to the default "cyro" theme.
    Useful for troubleshooting or reverting customizations.
    """
    _theme_manager.reset_to_defaults()