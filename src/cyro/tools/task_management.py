...
class TodoWriteRequest(BaseModel):
    ...
    todos: List[Dict[str, Any]] = Field(
        description="List of todo items to create/update"
    )
...
class TaskDelegationRequest(BaseModel):
    ...
    requirements: List[str] = Field(
        default_factory=list, description="Task requirements/constraints"
    )
...
class TaskManager:
    ...
    def delegate_task(self, request: TaskDelegationRequest) -> TaskDelegationResult:
        ...
        if any(
            ...
            for keyword in ["react", "vue", "ui", "frontend", "css", "html"]
        ):
        ...
        # Backend keywords
            ...
            keyword in task_lower
            for keyword in ["api", "backend", "server", "database", "sql"]
        ):
        ...
        # Data keywords
            ...
            keyword in task_lower
            for keyword in ["data", "analysis", "ml", "model", "analytics"]
        ):
        ...
        # DevOps keywords
            ...
            keyword in task_lower
            for keyword in ["deploy", "docker", "kubernetes", "ci/cd", "infrastructure"]
        ):
        ...
        if request.preferred_agent and request.preferred_agent in agent_scores:
            agent_scores[request.preferred_agent] = min(
                1.0, agent_scores[request.preferred_agent] + 0.3
            )
        ...
        delegated_todo = TodoItem(
            ...
            tags=["delegated"]
            + (["urgent"] if request.priority == TaskPriority.URGENT else []),
            estimated_duration=request.estimated_duration
            if hasattr(request, "estimated_duration")
            else None,
        )
        ...
        if request.deadline:
            ...
            estimated_completion = datetime.now() + timedelta(
                hours=hours_to_complete[request.priority]
            )
    ...
    def query_tasks(self, query: TaskQuery) -> TaskQueryResult:
        ...
        matching_tasks.sort(
            key=lambda t: (priority_order[t.priority], t.created_at), reverse=True
        )
...
def create_task_management_toolset(
